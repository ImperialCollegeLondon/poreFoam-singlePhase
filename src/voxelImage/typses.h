...
vars<dbls> distribution(const piece<T>  xs, const piece<T> ws, int nBins=64)
{
	...
	double deltaU=spanU/nBins+1.0e-72;
...
template<class T>
vars<dbls> distribution_uniA(const piece<T>  xs, const piece<T> ws, int nBins=64)
{
	vars<dbl2> xys;
	for (int i=0; i<ws.size(); ++i) { xys[i].second=ws[i]; xys[i].first=xs[i]; }
	sort(xys.begin(), xys.end(), [](dbl2 a, dbl2 b){ return a.first<b.first; });
	//for (auto d = xys.d+1; d<xys.dn; ++d) { d->second += (d-1)->second; } //! compute  cumulativeDF
	vars<dbls> distrib(3,dbls(nBins, 0.0));
	
	double Xold=xys.begin()->first;
	double delI=xys.begin()->second;
	double delCDF=1.0/nBins;
	int distInd=-1, iOld=0; 
	for (int i=1; i<xys.size(); ++i)
	{
		if(delI>=delCDF)
		{
			double Xnew=0.5*(xys[i].second + xys[i-1].second);
			distrib[0][++distInd]=0.5*(Xnew+Xold);
			distrib[2][distInd]=delI/std::max(Xnew-Xold,1.0e-32);//! avoid large values in PDF
			distrib[1][distInd]=i-iOld;
			Xold=Xnew;
			delI=xys[i].second;
			iOld=i;
		}
	...
	distrib[1]/=distrib[1].sum();
	distrib[2]/=distrib[2].sum();

	return distrib;
}
template<class T>
dbls distribution_bins(const piece<T>  xs, int nBins=64)
{//! uniform bins for probability density 
	dbls bins(nBins, 0.0);
	double minU=min(xs);
	double deltaU=(max(xs)-minU)/nBins+1.0e-72;
	for (int i=0; i<nBins; ++i)	bins[i] = minU+deltaU/2.0+i*deltaU;
	return bins;
}
template<class T>
dbls distribution_weis(const piece<T>  xs, const piece<T> ws, int nBins=64)
{ //! weighted probability density 
	dbls distrib(nBins, 0.0);
	double minU=min(xs);
	if(xs.size()!=ws.size()) std::cout<<"Error Size mismatch "<<xs.size()<<"!="<<ws.size()<<std::endl;
	double deltaU=(max(xs)-minU)/nBins+1.0e-72;
	for (int i=0; i<xs.size(); ++i)	distrib[std::min(int((xs[i]-minU)/deltaU),nBins-1)]+=double(ws[i]);
	//std::cout<<min(distrib)<<"   @@@@@  "<<ws[0]<<"   sum  "<<distrib.sum()<<"   sum  "<<ws.sum()<<std::endl;
	//distrib/=std::max(distrib.sum(),1.0e-64)*deltaU;
	return distrib;
}
template<class T>
dbls distribution_pdfs(const piece<T>  xs, int nBins=64)
{ //! probability density 
	dbls distrib(nBins, 0.0);
	double minU=min(xs);
	double deltaU=(max(xs)-minU)/nBins+1.0e-72;
	for (int i=0; i<xs.size(); ++i)	distrib[std::min(int((xs[i]-minU)/deltaU+0.5),nBins-1)]+=1.0;
	distrib/=distrib.sum()*deltaU;
	return distrib;
}
