...
template<typename T>
void voxelImageT<T>::setLayer(int k, const T* Values)
{
	//(*this)[k]=Values;
...
template<typename T>
void voxelImageT<T>::setSlice(char dir, int ijk, T vv)
{
	if(dir=='i')
...
//kji
template<typename T>
void voxelImageT<T>::setBlock(int n1, int n2, int n3, const voxelField<T>& Values)
{
	forAllkji(Values)
...
template<typename T>
void voxelImageT<T>::writeHeader(std::string outputName, std::string sufix) const
{
	this->voxelImageT<T>::writeHeader(outputName, int3{{0,0,0}}, voxelField<T>::sizeu3(),sufix);
}

template<typename T>
void voxelImageT<T>::writeHeader(std::string outName, int3 iStart, int3 iEnd, std::string sufix) const
{
...
void voxelImageT<T>::write(std::string outName) const
{
	...
	{
		this->writeBin(outName.substr(0,outName.size()-4)+".raw.gz");
		writeHeader(outName,".gz");
	}
	else if (outName.compare(outName.size()-4,4,".raw") == 0)
	{
