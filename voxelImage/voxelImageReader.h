...
{
	int X0(voximage.size3()[0]/2), Y0(voximage.size3()[1]/2);
	int R((X0+Y0)/2);
	char dir='z';
	inputs >> dir >> X0 >> Y0 >> R;
	(std::cout<<"  circleOut: dir="<<dir<<",  X0="<<X0 <<"  Y0="<<Y0  <<"  R="<<R ).flush();

	circleOut(voximage,X0,Y0,R,dir);
...
template<typename T> std::vector<std::pair<std::string,bool(*)( std::stringstream&, voxelImageT<T>&)> > namedProcesses()
{

	typedef bool(*ProcessP)( std::stringstream&  inputs, voxelImageT<T>& vxlImage);
	std::vector<std::pair<std::string,ProcessP> > name_Processes;
	name_Processes.push_back(std::pair<std::string,ProcessP>("",& ignore));
	//name_Processes.push_back(std::pair<std::string,ProcessP>("%"           , & ignore));
	//name_Processes.push_back(std::pair<std::string,ProcessP>("#"           , & ignore));
	name_Processes.push_back(std::pair<std::string,ProcessP>(";"           , & ignore));
	name_Processes.push_back(std::pair<std::string,ProcessP>("fillHoles"   , & fillHoles));
	name_Processes.push_back(std::pair<std::string,ProcessP>("pore"        , & selectPore));
	name_Processes.push_back(std::pair<std::string,ProcessP>("threshold"   , & selectPore));
	name_Processes.push_back(std::pair<std::string,ProcessP>("resample"    , & resample));
	name_Processes.push_back(std::pair<std::string,ProcessP>("Offset"   , & Offset));
	name_Processes.push_back(std::pair<std::string,ProcessP>("direction"   , & redirect));
	name_Processes.push_back(std::pair<std::string,ProcessP>("crop"        , & crop));
	name_Processes.push_back(std::pair<std::string,ProcessP>("cropD"       , & cropD));
	name_Processes.push_back(std::pair<std::string,ProcessP>("resampleMax" , & resampleMax));
	name_Processes.push_back(std::pair<std::string,ProcessP>("replaceRange", & replaceRange));
	name_Processes.push_back(std::pair<std::string,ProcessP>("write"  ,& write));
	name_Processes.push_back(std::pair<std::string,ProcessP>("medianFilter"  ,& medianFilter));
	name_Processes.push_back(std::pair<std::string,ProcessP>("circleOut"  ,& circleOut));
	name_Processes.push_back(std::pair<std::string,ProcessP>("maskWriteFraction"  ,& maskWriteFraction));

	return name_Processes;
}
...
)
{
	...
	std::vector<std::pair<std::string,ProcessP> > name_Processes = MCTProcessing::namedProcesses<T>();
	...
	if (processKeys)
	{
      ...
      while (true)
      {
			...
			headerFile>>tmpStr;
			bool validKey=false;
			for (unsigned int i=0; i<name_Processes.size();i++)
			{
				if (headerFile.fail()) break;
				...
				else if (tmpStr.compare(name_Processes[i].first)==0)
				{
					(std::cout<<" "<<tmpStr<<": ").flush();
					std::stringstream keywordData;
					headerFile.get (*(keywordData.rdbuf()));
					(*(name_Processes[i]).second)(keywordData,*this);
					std::cout<<std::endl;
					validKey=true;  break;
				}
			}
			...
			if ( !validKey )
			{
				if (tmpStr.size()) std::cout<<"  stoped reading "<<header<<" from entry \""<<tmpStr<<"\":/ "<<std::endl;
				else   std::cout<<" finished reading "<<header<<":/  "<<std::endl;
            headerFile.seekg(begLine);
            break;
			}
		}
